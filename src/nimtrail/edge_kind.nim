...
type
  ...
  SourcetrailEdgeKind_Impl* {.importcpp: "sourcetrail::EdgeKind".} = enum
  ...
  SourcetrailEdgeKind* = enum
    ekUNKNOWN, ekMEMBER, ekTYPE_USAGE, ekUSAGE, ekCALL, ekINHERITANCE, ekOVERRIDE,
    ekTYPE_ARGUMENT, ekTEMPLATE_SPECIALIZATION, ekINCLUDE, ekIMPORT,
    ekMACRO_USAGE, ekANNOTATION_USAGE
...
const
  arrSourcetrailEdgeKindmapping: array[SourcetrailEdgeKind, tuple[name: string,
      cEnum: SourcetrailEdgeKind_Impl, cName: string, value: int]] = [
...
proc `$`*(en: SourcetrailEdgeKind): string {.inline.} =
  arrSourcetrailEdgeKindmapping[en].cName
...
proc edgeKindToInt*(kind: SourcetrailEdgeKind): cint {.
